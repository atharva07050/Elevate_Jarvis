  ...
  "plans": [
    {
      ...
      "leetcodeLink": "string (A valid LeetCode problem search link or URL for this topic, if applicable)",
...
`;
  ...
  try {
    const json = await executeWithFallbackJSON(prompt);
    let plans = json.plans || (Array.isArray(json) ? json : [json]);
    if (plans.length > 0 && plans[0].plans) {
      plans = plans[0].plans;
    }
    return plans;
  } catch (err) {
    ...
    return topics.map(topic => ({
      ...
      leetcodeLink: `https://leetcode.com/problemset/all/?search=${encodeURIComponent(topic)}`,
